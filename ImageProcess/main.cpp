...
int main() {
  ...
  for (cl_uint i = 0; i < platformIdCount; ++i) {
    std::cout << "\t (" << (i + 1) << ") : " << GetPlatformName(platformIds[i])
              << std::endl;
  }
  ...
  for (cl_uint i = 0; i < deviceIdCount; ++i) {
    std::cout << "\t (" << (i + 1) << ") : " << GetDeviceName(deviceIds[i])
              << std::endl;
  }
  ...
  cl_context context =
      clCreateContext(contextProperties, deviceIdCount, deviceIds.data(),
  ...
  cl_mem inputImage =
      clCreateImage2D(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, &format,
  ...
  cl_mem outputImage =
      clCreateImage2D(context, CL_MEM_WRITE_ONLY, &format, image.width,
                      image.height, 0, nullptr, &error);
  CheckError(error);
  ...
  cl_command_queue queue =
      clCreateCommandQueue(context, deviceIds[0], 0, &error);
  CheckError(error);
  ...
  CheckError(clEnqueueNDRangeKernel(queue, kernel, 2, offset, size, nullptr, 0,
                                    nullptr, nullptr));
